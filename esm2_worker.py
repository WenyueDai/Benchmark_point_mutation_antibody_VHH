...
def main():
    ...
    tidy_df = pd.DataFrame(records, columns=[
        "chain", "pos", "wt", "mt",
        "delta_log_likelihood_esm2",
        "mut_log_likelihood_esm2",
        "wt_log_likelihood_esm2",
        "sample"
    ])

    tidy_path = os.path.join(".", f"{sample_name}_esm2_tidy.csv")
    tidy_df.to_csv(tidy_path, index=False)
    print(f"Wrote tidy ESM2 CSV to {tidy_path}")
